...
//MX1508 motorA(IN1_PIN, IN2_PIN, CH1, CH2);                    // do not use MotorA as it starts to move when booting (PIN15 pulses PWM at boot)
MX1508 motorB(IN3_PIN, IN4_PIN, CH1, CH2);                      // Default-  8 bit resoluion at 2500 Hz
//MX1508 motorB(IN1_PIN,IN2_PIN, CH1, CH2, RES);                // Specify resolution
//MX1508 motorB(IN1_PIN,IN2_PIN, CH1, CH2, RES, FREQ);          // Specify resolution and frequency
...
#define UP_PIN    33  //touch pin to open door (when sleeping)
#define DOWN_PIN  32  //touch pin to close door (when sleeping)
#define CONF_PIN  27  //touch pin to configure WIFI (touch and hold this pin and press reset button)
#define CALIB_UP  14  //touch pin to calibrate UP position (start when door closed, hold this pin, push reset button, release when door is opened)
bool calibrating = false;

#define IR_SENSOR     //comment this line if endstop switches instead of IR sensor
#ifdef IR_SENSOR
#define IR_OUT_PIN 23
#define PWR_PIN 19
...
int tops = 0;       //nb tops when opening/closing door (8 per motor turn)
int topMax = 30;     //number of pulses to get open position
...
bool touch7detected = false;  //touch7 used to launch WifiManager (hold it while reseting)
...
void setup()
{
  ...
  if (resetWake)
  {
    if (touchRead(T7) < threshold) touch7detected = true; //detect touchpad for CONFIG_PIN
    if (touchRead(T6) < threshold)
    {
      calibrating = true;
      chickenStatus = opening;
      Serial.print("calibration of opened position, touch detected ");
  ...
  tops = preferences.getInt("tops", 0);
  topMax = preferences.getInt("topMax", 30);
  topMax = max(5, topMax);                    //prevents blocking calibration if topMax = 0...

  strAscMargin = preferences.getString("channelsOrder", "RETAUL12");
  ...
  digitalWrite(PWR_PIN, LOW);                         //IR sensor is OFF
  attachInterrupt(IR_OUT_PIN, optical_ISR, RISING);   //will count tops on IR Sensor (8 per motor turn)
  IRtimeout = millis();
  ...
  if (resetWake && touch7detected)  //then launch WifiManager
  {
    //fetches ssid and pass and tries to connect
    //if it does not connect it starts an access point with the specified name
    //here  "AutoConnectAP"
    //and goes into a blocking loop awaiting configuration
    if (!wifiManager.startConfigPortal("JP ChickenDoor"))
    {
...
#ifdef IR_SENSOR
  digitalWrite(PWR_PIN, HIGH);                         //IR sensor is ON
  attachInterrupt(IR_OUT_PIN, optical_ISR, RISING);   //will count tops on IR Sensor (8 per motor turn)
  IRtimeout = millis();
#endif
  ...
  switch (chickenStatus)
  {
    case opening:
      ...
      if (tops < topMax)
      {
        if (calibrating)
        {
          if (touchRead(T6) < threshold) topMax = tops + 5; //door not yet opened, increase topMax
          else                                              //door now opened, stop calibration and save opened position
          {
            //delay(100);
...
#endif
#ifdef IR_SENSOR
  digitalWrite(PWR_PIN, LOW);   //switch off the IR sensor
#endif
